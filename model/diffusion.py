...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.use_ccl = use_ccl
        # hierarchical constraint loss config, e.g.
        # {"frames": (0, 75), "joints": [16, 17, 18, ...], "weight": 3.0}
        # frames=None -> all frames; joints=None -> all joints
    ...
    def _hierarchical_weights(self, s, c, device):
        """Build (1, s, c) channel weights and (1, s, 24, 1) joint weights
        implementing the hierarchical (time-segment x body-part) constraint."""
        cfg = self.hierarchical_loss
        w_extra = cfg.get("weight", 2.0) - 1.0
        frames = cfg.get("frames")
        joints = cfg.get("joints")

        time_mask = torch.zeros(1, s, 1, device=device)
        if frames is None:
            time_mask[:] = 1.0
        else:
            time_mask[:, frames[0] : frames[1]] = 1.0

        chan_mask = torch.zeros(1, 1, c, device=device)
        joint_mask = torch.zeros(1, 1, 24, 1, device=device)
        if joints is None:
            chan_mask[:] = 1.0
            joint_mask[:] = 1.0
        else:
            for j in joints:
                chan_mask[:, :, 7 + 6 * j : 13 + 6 * j] = 1.0
                joint_mask[:, :, j] = 1.0
            if 0 in joints:  # root also owns translation + contacts
                chan_mask[:, :, 0:7] = 1.0

        chan_w = 1.0 + w_extra * time_mask * chan_mask  # 1 x s x c
        joint_w = 1.0 + w_extra * time_mask.unsqueeze(-1) * joint_mask  # 1 x s x 24 x 1
        return chan_w, joint_w
    ...
    def p_losses(self, x_start, cond, t):
        ...
        # hierarchical constraint weights (time segment x body part)
        chan_w, joint_w = None, None
        if self.hierarchical_loss is not None:
            b_, s_, c_ = model_out.shape
            chan_w, joint_w = self._hierarchical_weights(s_, c_, model_out.device)
        ...
        foot_loss_weight = 10.942 if self.use_ccl else 0.0
        losses = (
            0.636 * loss.mean(),
            2.964 * v_loss.mean(),
            0.646 * fk_loss.mean(),
            foot_loss_weight * foot_loss.mean(),
        )
