...
class EDGE:
    def __init__(
        ...
        if checkpoint_path != "":
            # hierarchical models add new parameters that are absent from old
            # checkpoints, so load non-strictly in that case
            strict = not use_hierarchical
            ...
            # also seed the EMA (master) model from the checkpoint, otherwise the
            # EMA accumulator stays at random init and short fine-tunes produce
            # garbage ema_state_dict weights.
            self.diffusion.master_model.load_state_dict(
                checkpoint["ema_state_dict"], strict=strict
            )
