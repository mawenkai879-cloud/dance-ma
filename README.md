...
# Hierarchical EDGE &mdash; Hierarchical Music-to-Dance Generation

This repository extends the official **EDGE** dance-generation model with a
**hierarchical (global &rarr; local) conditioning** scheme and a
**hierarchical time-segment &times; body-part loss**, together with an evaluation
and ablation suite.

## What is new in this fork

- **Hierarchical conditioning (`model/model.py`, `DanceDecoder`).**
  The whole-clip music feature is mean-pooled and passed through a
  `global_encoder` MLP to produce a *global style* vector. This global style
  (1) FiLM-modulates the per-frame local music tokens, (2) is appended to the
  cross-attention memory, and (3) is added to the FiLM timestep condition.
  Classifier-free guidance is supported via a `null_global_token`. A
  `global_pool_override` hook allows constraining/swapping the global style at
  sampling time.
- **Hierarchical loss (`model/diffusion.py`, `_hierarchical_weights`).**
  A configurable per-(time-segment &times; body-part) weighting applied to the
  reconstruction and FK losses, controlled via
  `--hierarchical_loss_frames / --hierarchical_loss_joints / --hierarchical_loss_weight`.
- **Evaluation & ablation suite (`eval/`).** PFC, Beat Align, FID_k and Div_k
  metrics, t-SNE of the learned global style, a global style-swap experiment,
  and a controlled 2&times;2 / progressive ablation. Result figures and CSVs are
  under `eval/ablation/`.

### Key ablation result (controlled: 10-epoch fine-tune, loss weight 1.5)

| Variant | H-Cond | H-Loss | PFC &darr; | Beat Align &uarr; | FID_k &darr; | Div_k (&rarr;GT 10.21) |
|---|:---:|:---:|---|---|---|---|
| Baseline (EDGE) | - | - | 1.019 | 0.239 | 341.6 | 13.82 |
| + Hier. Conditioning | Y | - | 1.736 | 0.257 | 423.3 | 19.04 |
| + Hier. Loss | - | Y | 1.323 | 0.250 | 427.5 | 17.11 |
| **+ Both (full)** | **Y** | **Y** | **0.619** | **0.264** | **170.6** | 4.91 |

The full model attains the best PFC, Beat Align and FID_k, showing the two
...
> git-ignored. See the original instructions below to obtain them.
