...
def parse_train_opt():
    ...
    parser.add_argument(
        "--no_ccl",
        action="store_true",
        help="Ablate the Contact Consistency Loss (foot skate loss)",
    )
    parser.add_argument(
        "--use_hierarchical",
        action="store_true",
        help="Use global->local hierarchical music conditioning",
    )
    parser.add_argument(
        "--hierarchical_loss_frames",
        type=str,
        default=None,
        help='Frames range for constraint loss, e.g. "0,75" for first half',
    )
    parser.add_argument(
        "--hierarchical_loss_joints",
        type=str,
        default=None,
        help='Joint indices for constraint loss, e.g. "16,17,18,19,20"',
    )
    parser.add_argument(
        "--hierarchical_loss_weight",
        type=float,
        default=3.0,
        help="Extra loss weight for the constrained frames / joints",
    )
...
def parse_test_opt():
    ...
    parser.add_argument(
        "--use_hierarchical",
        action="store_true",
        help="Checkpoint uses hierarchical global->local music conditioning",
    )
    opt = parser.parse_args()
    return opt
