...
class DanceDecoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # hierarchical global (whole-song style) conditioning branch:
        # a global token summarizing the entire music clip guides the
        # per-frame (local) condition tokens via FiLM modulation
        if use_hierarchical:
            ...
            self.global_film = DenseFiLM(latent_dim)
    ...
    def forward(
        ...
        # hierarchical: global style summary modulates local condition tokens
        global_hidden = None
        if self.use_hierarchical:
            global_pool = cond_embed.mean(dim=1)
            override = getattr(self, "global_pool_override", None)
            if override is not None:
                global_pool = override.to(
                    global_pool.device, global_pool.dtype
                ).expand_as(global_pool)
            global_hidden = self.global_encoder(global_pool)  # b x d
            cond_tokens = featurewise_affine(
                cond_tokens, self.global_film(global_hidden)
            )
        ...
        if self.use_hierarchical:
            ...
            null_global_token = self.null_global_token.to(global_token.dtype)
            global_token = torch.where(
                keep_mask_embed, global_token, null_global_token
            )
            c = torch.cat((c, global_token), dim=-2)
            # also strengthen FiLM conditioning with the global style
            t = t + torch.where(
                keep_mask_hidden, global_hidden, torch.zeros_like(global_hidden)
            )
